...
// normalize scales a 3D vector so its length is exactly 1.0
...
// rotate3D uses Rodrigues' rotation formula to swing a point around a 3D axis
...
func main() {
	...
	// 1. Pick a random Expresso variation
	targetL := 20.0 + rng.NormFloat64()*5.0
	targetA := 12.0 + rng.NormFloat64()*3.0
	targetB := 18.0 + rng.NormFloat64()*4.0

	// 2. The geometric fix: anchor the rotation point halfway in lightness (50) and exactly zero chromaticity
	// The target point (Espresso) is around L=20. So the radius to 50 is ~30-40 units.
	// Therefore, swinging this radius around 50 means we orbit from ~10 up to ~90 lightness.
	// We'll never cross 0 and hit pure black, and we'll never cross 100 to pure white!
	centerL, centerA, centerB := 50.0, 0.0, 0.0

	// 3. Pick a random 3D rotation axis
	...
	// Since we know the point targetL, targetA, targetB sits on the orbit at step anchorIndex,
	// let's rotate BACKWARDS from there to find the true starting point of the walk (step 0).
